...
export class ReplacementComponent implements OnInit {
  ...
  exportexcel() {
    // Step 1: Prepare the data array from replacementList
    const data = this.replacementList.map((replacement, index) => ({
      'Sr.No': index + 1,
  ...
  uploadExcelData(data: any[]) {
    ...
    this.replacementService.uploadReplacement(formattedData).subscribe(response => {
      this.getAllReplacements();
