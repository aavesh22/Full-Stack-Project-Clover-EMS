...
export class NewRequirementComponent implements OnInit {
  ...
  exportexcel() {
    // Step 1: Prepare the data array from newRequirementList
    const data = this.newRequirementList.map((newRequirement, index) => ({
      'Sr.No': index + 1,
  ...
  uploadExcelData(data: any[]) {
    ...
    this.newRequirementService.uploadNewRequirements(formattedData).subscribe(response => {
      this.getAllNewRequirements();
